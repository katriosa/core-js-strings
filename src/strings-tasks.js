...
function removeFirstOccurrences(str, value) {
  ...
  const result = str.substring(0, index) + str.substring(index + value.length);
  return str.includes(value) ? result : str;
}
...
function isPalindrome(str) {
  const modifStr = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  return modifStr === modifStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  const result = arr.reduce((longWord, word) => {
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverseWordInArr = arr.map((word) => {
    return word.split('').reverse().join('');
  });
  return reverseWordInArr.join(' ');
}
